...
describe("Scoreboard", () => {
  ...
  describe("updateScore", () => {
    ...
    it("throws an error when updating score for non-existent match", () => {
      expect(() =>
        scoreboard.updateScore("35ec3f52-f61f-4d0a-aaed-88a256ec400f", 1, 1),
      ).toThrowError(expect.any(String));
    });
  });
  ...
  describe("finishMatch", () => {
    ...
    it("throws an error when finishing a non-existent match", () => {
      expect(() =>
        scoreboard.finishMatch("2e088d04-fe43-43b6-896d-1b25a54660a3"),
      ).toThrowError(expect.any(String));
    });
  });
